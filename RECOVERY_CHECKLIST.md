...
4. **Found and cleaned up a red herring**: a Compute Engine VM `gun-sounds-db` (e2-micro, us-west1-b) had been created 2026-07-18 mid-outage as an apparent panic workaround, paired with a Cloud Run revision (`gun-sounds-app-00058-kkg`) that pointed `DB_HOST` at the VM's internal IP. That revision **failed to deploy** and never got traffic — the whole time, the live revision (`gun-sounds-app-00057-rpx`, from March) correctly used the Cloud SQL socket path. The VM was never actually load-bearing; deleted it. The failed revision itself can't be deleted directly (Cloud Run won't delete the current "latest" revision even unused) but is harmless — would need a fresh redeploy to clean up cosmetically, not worth it on its own.
